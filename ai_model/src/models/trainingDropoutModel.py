...
class DropoutModelTrainer:
    ...
    def train(self, data):
        ...
        O target é a coluna 'dropout_label' (1 = Evadiu, 0 = Permaneceu).
        """
        if self.preprocessor is None:
            return

        # 1. Definir Target e Features
        if 'dropout_label' not in data.columns:
            print("❌ Erro: A coluna 'dropout_label' não está no dataset.")
            return

        X = data.drop(columns=['dropout_label'])
    ...
    def _save_model(self):
        joblib.dump(self.model, '../pipelines/dropout_logreg_model.pkl')
